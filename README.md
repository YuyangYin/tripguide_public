...
> 这是一个从私有项目 `tripguide` 开源改造而来的公开版本：移除了原有的 Supabase 多人共享后端，并用一份「冰岛 + 罗弗敦 12 天 · 香港往返」的示例行程替换了原作者的私人行程数据。
...
- 本项目**不连接任何远端数据库**，已彻底移除原版的 Supabase 依赖与实时同步逻辑。
- 你的行程、票根、记账全部保存在**当前浏览器**中。换设备 / 清缓存即不可见，也不会同步给其他人。
- 没有"云备份"概念：想要跨设备，请自行导出或截图保存。
- 示例行程（`src/components/ItineraryTimeline.tsx` 中的 `ITINERARY_DATA`）仅为演示数据，可自由修改为你自己的真实安排。
