...
- **每日天气与穿衣建议**：从 Open-Meteo 获取计划地点逐日预报，显示温度、体感、降雨、风力和穿衣建议，并标明预报日期与数据抓取日期；每天打开时自动刷新并共享给同行成员。
...
- **工具箱**：
  - 随手记账：固定四成员、付款人与分账人、个人支出、单笔账单支持全部结算或勾选部分已结算成员、按人净额结算、成员间已转账流水及 XLSX/XLS/CSV 批量导入；统计团队分类总额、每人总花费与每人的分类花费，账单流水可按分类、记录日期和结算状态组合筛选并按日查看。
