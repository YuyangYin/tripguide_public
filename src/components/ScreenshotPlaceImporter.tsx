...
export default function ScreenshotPlaceImporter({ cityHint, onApply }: {
  cityHint: string;
  onApply: (result: TravelScreenshotResult) => void;
}) {
  ...
  return <div className="rounded-2xl border border-cyan-400/20 bg-cyan-400/5 p-3">
    ...
    {result && <div className="mt-3 space-y-2">{(Object.keys(LABELS) as ResultKey[]).map((key) => <label key={key} className="block text-[9px] font-bold text-white/60">{LABELS[key]} · {result[key].length} 项<textarea rows={Math.max(2, Math.min(4, result[key].length + 1))} value={result[key].join('\n')} onChange={(event) => update(key, event.target.value)} placeholder={`没有识别到${LABELS[key]}，可手动补充`} className="mt-1 w-full rounded-lg border border-white/10 bg-black/25 px-2 py-1.5 text-[10px] text-white" /></label>)}<div className="flex gap-2"><button type="button" onClick={() => setResult(null)} className="flex flex-1 items-center justify-center gap-1 rounded-lg bg-white/10 py-2 text-[10px] font-bold"><X className="h-3 w-3" />取消</button><button type="button" onClick={() => { onApply(result); setResult(null); setStatus(`已加入 ${cityHint} 当天编辑表单，点击底部保存并同步`); }} className="flex-[2] rounded-lg bg-cyan-300 py-2 text-[10px] font-black text-slate-950">导入到 {cityHint || '当天行程'}</button></div></div>}
  </div>;
}
